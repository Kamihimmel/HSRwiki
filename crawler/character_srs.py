...
def append_image(character, data):
    ...
    path = util.download_image(path, data['splashIconPath'], 'webp', base_dir)
    result['imageurl'] = path
    path = 'images/characters/%slarge' % character
    path = util.download_image(path, data['artPath'], 'webp', base_dir)
...
def append_skill_image(character, cur_skill, skill_en, index):
    ...
    path = util.download_image(path, skill_en[index]['iconPath'], 'webp')
    cur_skill['imageurl'] = path

def append_skill_desc(cur_skill, skill_dict, index):
    for lang in languages:
        cur_skill['Description' + lang] = util.clean_desc(skill_dict[lang][index]['descHash'])
...
def append_skill_attr(cur_skill, skill_en, index):
    ...
    maxlevel = len(skill_en[index]['levelData'])
    if maxlevel > 1:
        cur_skill['maxlevel'] = maxlevel
    else:
        cur_skill['maxlevel'] = 0
    cur_skill['buffskill'] = util.is_buff_skill(cur_skill['DescriptionEN'])
    cur_skill['teamskill'] = util.is_team_skill(cur_skill['DescriptionEN'])
    ...
    if len(skill_levels) == 1:
        for lang in languages:
            ...
            for i in range(0, placeholder_cnt):
                multiplier = skill_levels[0]['params'][i]
                placeholder = '[%s]' % (i + 1)
                if (placeholder + '%') in desc:
                    multiplier = util.format_percent_number(multiplier)
...
def append_trace_name_and_desc(character, cur_trace, trace_dict, index, img_cnt):
    ...
    for lang in languages:
        ...
        if cur_trace['tiny'] is False:   
            ...
            desc = util.clean_desc(trace_dict[lang][index]['embedBonusSkill']['descHash'])
            ...
            multiplier = util.format_percent_number(statusList[0]['value'])
    ...
    if asset_id is not None:
        ...
        path = util.download_image(path, asset_id, 'webp', base_dir)
...
def append_trace_attr(cur_trace):
    ...
    cur_trace['buffskill'] = util.is_buff_skill(cur_trace['ENname']) or util.is_buff_skill(cur_trace['DescriptionEN'])
    cur_trace['teamskill'] = util.is_team_skill(cur_trace['ENname']) or util.is_team_skill(cur_trace['DescriptionEN'])
...
def flat_trace_tree(trace_tree, trace_list):
    for t in trace_tree:
        trace_list.append(t)
        if len(t['children']) > 0:
            flat_trace_tree(t['children'], trace_list)
...
def append_eidolon_image(character, cur_eidolon, skill_en, index):
    ...
    path = util.download_image(path, skill_en[index]['iconPath'], 'webp', base_dir)
    cur_eidolon['imageurl'] = path

def append_eidolon_attr(cur_eidolon, eidolon_dict, index):
    for lang in languages:
        desc = util.clean_desc(eidolon_dict[lang][index]['descHash'])
...
def genrate_json(character):
    print('generate lib json from starrailstation for: %s' % character)
    c = character.replace('-', '')
    data_dict = {}
    for lang in languages:
